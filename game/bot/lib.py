...
class Vertex:
    ...
    def is_mine(self) -> bool:
        ...
        return self.controller is not None and self.controller == self.my_player_id
...
class Game:
    ...
    def can_capture(self, from_vertex: int, to_vertex: int, units: int) -> bool:
        ...
        if target.is_enemy:
            ...
            raise ValueError("Vertex is neither enemy, neutral, nor mine!?")

    def calculate_movement_cost(self, from_vertex: int, to_vertex: int) -> float:
...
class GameBot(ABC):
    ...
    def __init__(self, game_id: str | None = None, player_id: Optional[str] = None, server_url: str = "ws://localhost:8765"):
        """
        Initialize the bot.
