export class GameViewer {
    constructor() {
        ...
        this.maxTurns = 10; // Default max turns
...
sendForceStart() {
    ...
    updateUIWithGameId() {
        ...
        const titleText = isDefaultGame ? 'Chromatic Warfare' : `Chromatic Warfare - ${this.gameId}`;
        ...
        this.movingUnits.forEach(unit => {
            ...
            if (unit.combatResult === 'survivor' && unit.finalX !== undefined) {
                ...
                if (progress <= 0.5) {
                    ...
                    unit.currentX = unit.startX + (unit.endX - unit.startX) * combatProgress;
                    unit.currentY = unit.startY + (unit.endY - unit.startY) * combatProgress;
                    unit.alpha = 1.0;
                    ...
                    if (unit.survivorCount > 0) {
                        ...
                        unit.currentX = unit.endX + (unit.finalX - unit.endX) * survivalProgress;
                        unit.currentY = unit.endY + (unit.finalY - unit.endY) * survivalProgress;
                        unit.alpha = 1.0;
...
cleanupDisconnectedPlayerColors(currentPlayers) {
    ...
    handleTurnProcessed(turnData) {
        ...
        if (turnData.turn_duration_seconds !== undefined) {
            ...
            document.getElementById('turnDuration').textContent = turnData.turn_duration_seconds.toFixed(1);
        }
        
        if (turnData.max_turns !== undefined) {
            this.maxTurns = turnData.max_turns;
            document.getElementById('turnProgress').textContent = ` / ${this.maxTurns}`;
        }
    ...
    setupMovingUnits(moveAnimations) {
        ...
        edgeMoves.forEach((moves, edgeKey) => {
            if (moves.length === 1) {
                ...
                const pos1 = this.getVertexPosition(move1.from_vertex, scale, offsetX, offsetY);
                const pos2 = this.getVertexPosition(move1.to_vertex, scale, offsetX, offsetY);
                ...
                if (pos1 && pos2) {
                    ...
                    this.movingUnits.push({
                        ...
                        currentY: pos1.y,
                        alpha: 1.0,
                        combatResult: survivors1 > 0 ? 'survivor' : 'eliminated',
                    ...
                    this.movingUnits.push({
                        ...
                        currentY: pos2.y,
                        alpha: 1.0,
                        combatResult: survivors2 > 0 ? 'survivor' : 'eliminated',
    ...
    updateGameState(gameState) {
        ...
        if (gameState.turn_duration_seconds !== undefined) {
            ...
            document.getElementById('turnDuration').textContent = gameState.turn_duration_seconds.toFixed(1);
        }
        ...
        if (wasFirstState || !this.isAnimating) {
            ...
            if (wasFirstState && gameState.game_status === 'active') {
                this.addLogEntry('🎮 Game started! Players are making their moves.', 'game-event');
            }
...
if (vertex.sizeMultiplier) {
    ...
    renderMovingUnits() {
        ...
        this.movingUnits.forEach(unit => {
            if (unit.alpha <= 0 || (unit.displayedUnits !== undefined && unit.displayedUnits <= 0)) return;
            
            const color = this.playerColors[unit.playerId] || '#ffffff';
            const displayUnits = unit.displayedUnits !== undefined ? unit.displayedUnits : unit.units;
            ...
            if (unit.combatResult && unit.combatResult !== 'survivor' && unit.alpha > 0.5) {
            ...
            if (displayUnits > 0) {
                ...
                ctx.textAlign = 'center';
                ctx.textBaseline = 'middle';
                ctx.fillText(displayUnits.toString(), unit.currentX, unit.currentY);
            }
...
updatePlayers(players) {
    ...
    showGameResult(rankings) {
        ...
        this.addLogEntry('🏆 Game completed! Final rankings displayed in players list.', 'game-event');
